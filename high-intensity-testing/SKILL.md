...
1. **分治**:把审查拆成**相互独立的审查遍(pass)**——每遍独立上下文 + 详细任务书,互不知晓其他遍的结论(避免锚定)。运行时支持并行子代理就按模块拆 3–4 个并行;不支持子代理/并行/新会话的运行时,退化为**顺序多遍**(契约遍 → 数据流遍 → git 考古遍 → 对抗遍),遍间重置上下文。独立性是本质要求,并行只是实现手段。
